...
for mu_cnt = 1:length(mus)
    for kT_cnt = 1:length(kTs)
        ...
        X1 = randi(L, [MCS, MR], "uint32");
        Y1 = randi(L, [MCS, MR], "uint32");
        X2 = randi(L, [MCS, N*MR], "uint32");
        Y2 = randi(L, [MCS, N*MR], "uint32");
        
        % Nearest and next nearest neighbour arrays with periodic boundaries
        XP11 = X1+1-uint32(L*(X1==L)); 
        XP21 = X1+2-uint32(L*(X1>=L-1));
        YP11 = Y1+1-uint32(L*(Y1==L)); 
        YP21 = Y1+2-uint32(L*(Y1>=L-1));
        XM11 = X1-1+uint32(L*(X1==1));         
        XM21 = X1-2+uint32(L*(X1<=2));
        YM11 = Y1-1+uint32(L*(Y1==1)); 
        YM21 = Y1-2+uint32(L*(Y1<=2));
        
        XP12 = X2+1-uint32(L*(X2==L)); 
        XP22 = X2+2-uint32(L*(X2>=L-1));
        YP12 = Y2+1-uint32(L*(Y2==L)); 
        YP22 = Y2+2-uint32(L*(Y2>=L-1));
        XM12 = X2-1+uint32(L*(X2==1)); 
        XM22 = X2-2+uint32(L*(X2<=2));
        YM12 = Y2-1+uint32(L*(Y2==1)); 
        YM22 = Y2-2+uint32(L*(Y2<=2));
            ...
            for i = 1 : N % start of evaporation/condensation loop       
                if NP(X1(m, i),Y1(m, i)) == 0     
                    if LQ(X1(m, i),Y1(m, i)) == 0
                        ...
                        dE = double(-(LQ(XP11(m, i),Y1(m, i)) + LQ(XM11(m, i),Y1(m, i)) + LQ(X1(m, i),YP11(m, i)) + LQ(X1(m, i),YM11(m, i))))...
                            -e_nl*double(NP(XP11(m, i),Y1(m, i)) + double(NP(XM11(m, i),Y1(m, i)) + NP(X1(m, i),YP11(m, i)) + NP(X1(m, i),YM11(m, i)))) + mu;
